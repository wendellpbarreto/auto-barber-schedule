...
// Recurrence: Thu 12h → Tue 12h → Sat 9h, starting Thursday 05/02. Cycle: 5, 4, 5 days.
...
async function bookOneSlot(token, slot) {
...
async function main() {
  try {
    ...
    for (let i = 0; i < slots.length; i++) {
      ...
      try {
        const out = await bookOneSlot(authResult.token, slots[i]);
        if (out.booked) {
          results.booked.push({ slot: slots[i], data: out.booked });
          console.log("Booked:", slots[i].age_inicio);
        }
        if (out.alreadyScheduled) {
          results.alreadyScheduled.push({ slot: slots[i] });
          console.log("Already scheduled:", slots[i].age_inicio);
        }
      } catch (err) {
        results.errors.push({ slot: slots[i], error: err.message });
        console.error("Error for", slots[i].age_inicio, ":", err.message);
      }
    }
